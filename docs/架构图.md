# ⚠️ 本文档已作废（2026-09-19）——请勿据此理解架构

> **以 [`ARCHITECTURE.md`](../ARCHITECTURE.md) 为准。** 本文件仅作历史参考保留。
>
> 作失效原因（2026-09-19 全仓核实）：
>
> 1. **「🌱 养成系统」整块子图（约 30+ 节点）在代码中不存在。**
...
>    代码里留着弃用空壳注释：`pet.py:1358`、`pet_mixins/interaction_mixin.py:526`。
> 2. **mixin 数过时**：实际 12 个（本文漏 `PanelsMixin` / `PerchMixin`）。
> 3. **QThread 虚报**：文档称 5 处 Qt 侧后台，实际 0 处实例化。
> 4. **规模数字全漂**：见 `ARCHITECTURE.md` §15「文档漂移台账」。
>
> 完整偏差清单见 `ARCHITECTURE.md` §15。
